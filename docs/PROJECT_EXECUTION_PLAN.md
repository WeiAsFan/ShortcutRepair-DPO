...
> - 状态：已完成并归档
> - 起草日期：2026-08-31
> - 训练提交：`1ead3b24f00f33569128a6634401729e4908a62f`
> - FP32 评测提交：`91dc2b63fe3a07b417c27392e5dde2e8f39138d6`
> - 正式结果提交：`70618ae319fa92294738b54d581f2a9db6f85a98`
> - 首次结果提交：`6f9d8599588d7e5fb1754037494b0ce71d351a2d`
> - 目标实验版本：`v1.1`

> 本文保留 v1.1 从失败复盘到正式结果的原始执行计划。计划已经完成，正文中的待办框不再作为当前任务列表；v1.2 使用独立执行计划。

## 1. 文档目的

本文是 ShortcutRepair-DPO 从首次失败实验走向可信正式结果的执行指南。它规定修改顺序、实验冻结方式、验收标准、停止条件和最终交付物，避免在修复过程中同时改变过多变量，也避免为了得到正结果而在 test 上反复调参。

本计划适用于项目 `v1.1`。现有实现和 2026-08-28 的首次 A6000 运行视为 `v1.0` 基线。`v1.0` 的失败结果保留在 `main`，不覆盖、不删除，也不复制进 v1.1 分支；它仍可作为训练系统调试和实验复盘证据。
